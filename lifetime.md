...
```
メソッド名の後に<'a, 'b>を追加しました。これは、2つのライフタイムパラメータ、「aとb」を導入します。次に引数と戻り値の`＆`の後にライフタイム名を追加することで、ライフタイムパラメータと関連付けます。  
戻り値のライフタイムパラメータは`'a`なので`line`と同じ有効期間であるとわかるので
`p`がスコープから外れた後でも`v`参照を安全に使用できるようになります。

---
## シンタックス

`ライフタイム`とは参照の有効なスコープを記述するものです。
```Rust
// 黙示的に
fn foo(x: &i32) {
}
...
`<'a>`は`bar`がライフタイム`'a`を持つことを意味します。　　
引数リストでは、名付けたライフタイムを使います。
```Rust
fn bar<'a>(x: &'a i32)
```
...
参照を含む`struct`を使うときは、明示的なライフタイムが必要です。
```Rust
struct Foo<'a> {
    x: &'a i32, // ライフタイムを指定しなければコンパイルエラー
}
fn main() {
    let y = 5;
    let f = Foo { x: &y };
...
```
`Foo`への参照が`Foo`が含む参照より長い間有効にはなってはならないのでライフタイムを示す必要があります。
...
```
`Foo`のライフタイムは`impl`行で宣言する必要があります。  
impl<'a> はライフタイム 'a を定義し、 Foo<'a> がそれを使用します。
...
```
見てのとおり、ここでは`f`と`y`のスコープは`x`のスコープよりも小さいです。しかし`x = &f.x`を実行すると、スコープから外れた何かの参照にしてしまいます。

## 'static
`static`と名付けられたライフタイムは特別でプログラム全体に渡るライフタイムを持つことを示します。  
文字列リテラルは`&'static str`型を持ちます。
```Rust
let x: &'static str = "Hello, world.";
```
なぜなら、参照が常に有効だからです。文字列リテラルは最終的なバイナリのデータセグメントに焼き付けられます。  
もう1つの例はグローバルです。
```Rust
static FOO: i32 = 5;
let x: &'static i32 = &FOO;
```
これはバイナリのデータセグメントに`i32`を追加していて`x`はそれへの参照です。

## ライフタイムの省略
Rustは関数本体の部分では強力なローカル型推論をサポートしますが、要素のシグネチャの部分では、型が要素のシグネチャだけでわかるようにするため、型推論が許されていません。 とはいえ、エルゴノミック（人間にとっての扱いやすさ）の理由により、"ライフタイムの省略"と呼ばれている、非常に制限された第二の推論アルゴリズムがシグネチャの部分に適用されます。 その推論はシグネチャのコンポーネントだけに基づき、関数本体には基づかず、ライフタイムパラメータだけを推論します。そして3つの覚えやすく明確なルールに従って行います。ライフタイムの省略で要素のシグネチャを短く書くことができます。しかしローカル型推論が適用されるときのように実際の型を隠すことはできません。
